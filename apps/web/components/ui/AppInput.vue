...
<template>
  <div v-if="withClipboard" class="flex items-center gap-1">
    <input
      v-bind="$attrs"
      :type="type ?? 'text'"
      :value="modelValue"
      ...
      :class="[
        'flex-1 min-w-0 bg-transparent border-b border-dt-border focus:border-dt-accent',
        'outline-none py-1 text-sm text-dt-text placeholder:text-dt-dim transition-colors',
        mono ? 'font-mono' : '',
      ]"
  ...
  <input
    ...
    :placeholder="placeholder"
    :class="[
      'w-full bg-transparent border-b border-dt-border focus:border-dt-accent',
      'outline-none py-1 text-sm text-dt-text placeholder:text-dt-dim transition-colors',
      mono ? 'font-mono' : '',
    ]"
    @input="$emit('update:modelValue', ($event.target as HTMLInputElement).value)"
